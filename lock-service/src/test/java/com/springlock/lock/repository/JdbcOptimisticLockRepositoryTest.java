...
class JdbcOptimisticLockRepositoryTest {

    @Mock
    JdbcTemplate jdbcTemplate;

    JdbcOptimisticLockRepository repo;

    @BeforeEach
    void setUp() {
        repo = new JdbcOptimisticLockRepository(jdbcTemplate);
    }
    ...
    @Test
    void tryAcquire_whenInsertFailsAndRowNotFound_retriesInsertSuccessfully() {
    ...
    @Test
    void tryAcquire_whenInsertFailsAndRowNotFound_retriesInsertFails() {
    ...
    @Test
    void tryAcquire_whenInsertFailsAndRowExpired_updateSucceeds() {
        ...
        doAnswer(inv -> {
            ...
            when(rs.getString(1)).thenReturn("other");
            when(rs.getTimestamp(2)).thenReturn(Timestamp.from(expiredAt));
            when(rs.getLong(3)).thenReturn(1L);
            return rse.extractData(rs);
        }).when(jdbcTemplate).query(anyString(), any(ResultSetExtractor.class), anyString());
        when(jdbcTemplate.update(anyString(), any(), any(), anyString(), any(), any(Long.class))).thenReturn(1);

        boolean result = repo.tryAcquire("key", "owner", now, expiresAt);
    ...
    void tryAcquire_whenInsertFailsAndRowExpired_butUpdateFails_returnsFalse() {
        ...
        when(jdbcTemplate.update(anyString(), any(), any(), anyString(), any(), any(Long.class))).thenReturn(0);
    ...
    @Test
    void tryAcquire_whenInsertFailsAndRowNotExpired_returnsFalse() {
        ...
        doAnswer(inv -> {
            ...
            when(rs.next()).thenReturn(true);
            when(rs.getString(1)).thenReturn("other");
            when(rs.getTimestamp(2)).thenReturn(Timestamp.from(future));
            when(rs.getLong(3)).thenReturn(1L);
            return rse.extractData(rs);
        }).when(jdbcTemplate).query(anyString(), any(ResultSetExtractor.class), anyString());
    ...
    void tryAcquire_whenRowExpiresExactlyAtNow_returnsFalse() {
        ...
        doAnswer(inv -> {
            ...
            when(rs.next()).thenReturn(true);
            when(rs.getString(1)).thenReturn("other");
            when(rs.getTimestamp(2)).thenReturn(Timestamp.from(now));
            when(rs.getLong(3)).thenReturn(1L);
            return rse.extractData(rs);
        }).when(jdbcTemplate).query(anyString(), any(ResultSetExtractor.class), anyString());
    ...
    @Test
    void findLockInfo_whenActive_returnsLockInfo() {
    ...
    @Test
    void findLockInfo_whenExpired_returnsEmpty() {
        Instant expiredAt = Instant.EPOCH;
        doAnswer(inv -> {
            ResultSetExtractor<?> rse = inv.getArgument(1);
            ResultSet rs = mock(ResultSet.class);
            when(rs.next()).thenReturn(true);
    ...
    @Test
    void findLockInfo_whenNoRow_returnsEmpty() {
        doAnswer(inv -> {
            ResultSetExtractor<?> rse = inv.getArgument(1);
            ResultSet rs = mock(ResultSet.class);
            when(rs.next()).thenReturn(false);
    ...
    @Test
    void findVersion_whenFound_returnsVersion() {
        doAnswer(inv -> {
            ResultSetExtractor<?> rse = inv.getArgument(1);
            ResultSet rs = mock(ResultSet.class);
            when(rs.next()).thenReturn(true);
        ...
        assertThat(repo.findVersion("key")).hasValue(42L);
    }

    @Test
    void findVersion_whenNotFound_returnsEmpty() {
        doAnswer(inv -> {
            ResultSetExtractor<?> rse = inv.getArgument(1);
            ResultSet rs = mock(ResultSet.class);
            when(rs.next()).thenReturn(false);
